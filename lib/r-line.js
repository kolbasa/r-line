 ...
 * @returns {string}
 */
function previewChangedLine(lineNumber, originalLine, changedLine) {
    ...
    const content = previewContent(changedLine, paddingLength, changeIndicator);
 ...
 * @returns {String}
 */
function preview(originalLines, changedLines, options) {
    return changedLines
 ...
 * @returns {string[]}
 */
function applyUserChanges(changedLines, onLineCallback) {
    const nextLines = changedLines.slice(0); // array copy
    const previousLines = [];

    return changedLines.map((line, index) => {
        nextLines.shift();
