 ...
 * @returns {string}
 */
function applyRelativeIndentation(line, lineNumber, indentation) {
    if (line == null || line.trim == null) {
        return line;
    }
    let trim = indentation[lineNumber - 1];
    const newTrim = line.length - line.trimStart().length;
    if (trim == null || newTrim < trim) {
        trim = newTrim;
        indentation[lineNumber] = trim;
    }
...
function previewLine(lineChanged, lineOriginal, lineNumber, lineCount, indentation, options) {
    ...
    if (indentation != null && lineOriginal !== lineChanged) {
        if (lineChanged.trim != null) {
            lineChanged = lineChanged.split(N).map((line) => {
                return applyRelativeIndentation(line, lineNumber, indentation)
            }).join(N);
        }
        lineOriginal = applyRelativeIndentation(lineOriginal, lineNumber, indentation);
    }
...
function preview(linesOriginal, linesChanged, options) {
    ...
    const indentation = options.keepOriginalIndentation ? null : {};
    return linesChanged
        .map((lineChanged, i) => {
