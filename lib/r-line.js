 ...
 */
function previewOriginalLine(lineNumber, originalLine, modifiedLine) {
...
function previewChangedLine(lineNumber, originalLine, modifiedLine) {
    ...
    conf = originalLine == null ? conf : previewOriginalLine(...arguments);

    const paddingLength = lineNumber.toString().length;
    const content = previewContent(modifiedLine, paddingLength, conf.changeIndicator);
...
function listFiles(dirPath) {
    const files = [];
    iterateFiles(dirPath, (path) => files.push(path));
    return files;
}
 ...
 * @returns {void}
 */
function iterateFiles(dirPath, onFile) {
    fs.readdirSync(dirPath).forEach((file) => {
        if (fs.statSync(dirPath + '/' + file).isDirectory()) {
            iterateFiles(dirPath + '/' + file, onFile);
        } else {
            onFile(dirPath + '/' + file);
        }
    });
